...
pub(crate) fn execute(store: &StateStore, command: Commands) -> Result<(), AppError> {
    match command {
        ...
        Commands::Config(args) => handle_config(store, &args),
...
fn handle_inspect(store: &StateStore, args: &InspectArgs) -> Result<(), AppError> {
    ...
    let doc = store.update::<_, _, AppError>(|state| {
        ...
        let log_tail = read_log_tail(session.log_path.as_deref(), tail_lines).unwrap_or_default();
...
fn handle_start_spec(store: &StateStore, mut spec: LaunchSpec) -> Result<(), AppError> {
...
fn handle_config(store: &StateStore, args: &ConfigArgs) -> Result<(), AppError> {
    match &args.command {
    ...
    if output::is_json_mode() {
        let items = state
            ...
            .map(|(name, spec)| {
                json!({
                    "name": name,
                    "runtime": runtime_label(&spec.runtime),
...
fn read_log_tail(path: Option<&str>, max_lines: usize) -> Result<String, AppError> {
    if max_lines == 0 {
...
fn build_profile_spec(args: &ConfigSaveArgs) -> Result<LaunchSpec, AppError> {
    let runtime = to_runtime_kind(&args.runtime);
    let mode = to_launch_mode(&args.mode);
...
fn parse_env_map(items: &[String]) -> Result<BTreeMap<String, String>, AppError> {
...
fn to_launch_mode(mode: &LaunchModeArg) -> LaunchMode {
    match mode {
