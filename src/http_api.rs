...
use crate::http_utils::{
    http_is_authorized, http_json, http_json_error, http_path_segments, http_query_u64,
...
pub(crate) fn response_for_request(
    ...
    match (request.method(), segments.as_slice()) {
        ...
        (&tiny_http::Method::Get, ["v1", "sessions", session_id, "debug", "threads"]) => {
            http_handle_debug_threads(store, serve_state, session_id)
        }
        (&tiny_http::Method::Post, ["v1", "sessions", session_id, "debug", "breakpoints"]) => {
            http_handle_debug_breakpoints(store, serve_state, session_id, request)
        }
        (
        ...
        ) => http_handle_debug_exception_breakpoints(store, serve_state, session_id, request),
        (&tiny_http::Method::Post, ["v1", "sessions", session_id, "debug", "evaluate"]) => {
            http_handle_debug_evaluate(store, serve_state, session_id, request)
        }
        (&tiny_http::Method::Post, ["v1", "sessions", session_id, "debug", "set-variable"]) => {
            http_handle_debug_set_variable(store, serve_state, session_id, request)
        }
        (&tiny_http::Method::Post, ["v1", "sessions", session_id, "debug", "continue"]) => {
            http_handle_debug_continue(store, serve_state, session_id, request)
        }
        (&tiny_http::Method::Post, ["v1", "sessions", session_id, "debug", "pause"]) => {
            http_handle_debug_pause(store, serve_state, session_id, request)
        }
        (&tiny_http::Method::Post, ["v1", "sessions", session_id, "debug", "next"]) => {
            http_handle_debug_next(store, serve_state, session_id, request)
        }
        (&tiny_http::Method::Post, ["v1", "sessions", session_id, "debug", "step-in"]) => {
            http_handle_debug_step_in(store, serve_state, session_id, request)
        }
        (&tiny_http::Method::Post, ["v1", "sessions", session_id, "debug", "step-out"]) => {
            http_handle_debug_step_out(store, serve_state, session_id, request)
        }
        (&tiny_http::Method::Post, ["v1", "sessions", session_id, "debug", "disconnect"]) => {
            http_handle_debug_disconnect(store, serve_state, session_id, request)
        }
        (&tiny_http::Method::Post, ["v1", "sessions", session_id, "debug", "terminate"]) => {
            http_handle_debug_terminate(store, serve_state, session_id, request)
        }
        (&tiny_http::Method::Post, ["v1", "sessions", session_id, "debug", "adopt-subprocess"]) => {
            http_handle_debug_adopt_subprocess(store, serve_state, session_id, request)
        }
        (&tiny_http::Method::Get, ["v1", "sessions", session_id, "debug", "dap", "events"]) => {
            http_handle_dap_events(store, serve_state, session_id, query)
        }
        (&tiny_http::Method::Post, ["v1", "sessions", session_id, "debug", "dap", "request"]) => {
            http_handle_dap_request(store, serve_state, session_id, request)
        }
...
fn http_handle_debug_breakpoints(
    store: &StateStore,
    serve_state: &Arc<Mutex<DapRegistry>>,
    session_id: &str,
    request: &mut tiny_http::Request,
...
fn http_handle_debug_continue(
    store: &StateStore,
    serve_state: &Arc<Mutex<DapRegistry>>,
    session_id: &str,
    request: &mut tiny_http::Request,
) -> tiny_http::Response<std::io::Cursor<Vec<u8>>> {
    http_handle_debug_thread_control(store, serve_state, session_id, request, "continue")
}

fn http_handle_debug_exception_breakpoints(
    store: &StateStore,
    serve_state: &Arc<Mutex<DapRegistry>>,
    session_id: &str,
    request: &mut tiny_http::Request,
...
fn http_handle_debug_set_variable(
    store: &StateStore,
    serve_state: &Arc<Mutex<DapRegistry>>,
    session_id: &str,
    request: &mut tiny_http::Request,
...
fn http_handle_debug_pause(
    store: &StateStore,
    serve_state: &Arc<Mutex<DapRegistry>>,
    session_id: &str,
    request: &mut tiny_http::Request,
) -> tiny_http::Response<std::io::Cursor<Vec<u8>>> {
    http_handle_debug_thread_control(store, serve_state, session_id, request, "pause")
}

fn http_handle_debug_next(
    store: &StateStore,
    serve_state: &Arc<Mutex<DapRegistry>>,
    session_id: &str,
    request: &mut tiny_http::Request,
) -> tiny_http::Response<std::io::Cursor<Vec<u8>>> {
    http_handle_debug_thread_control(store, serve_state, session_id, request, "next")
}

fn http_handle_debug_step_in(
    store: &StateStore,
    serve_state: &Arc<Mutex<DapRegistry>>,
    session_id: &str,
    request: &mut tiny_http::Request,
) -> tiny_http::Response<std::io::Cursor<Vec<u8>>> {
    http_handle_debug_thread_control(store, serve_state, session_id, request, "stepIn")
}

fn http_handle_debug_step_out(
    store: &StateStore,
    serve_state: &Arc<Mutex<DapRegistry>>,
    session_id: &str,
    request: &mut tiny_http::Request,
) -> tiny_http::Response<std::io::Cursor<Vec<u8>>> {
    http_handle_debug_thread_control(store, serve_state, session_id, request, "stepOut")
}

fn http_handle_debug_thread_control(
    store: &StateStore,
    serve_state: &Arc<Mutex<DapRegistry>>,
    session_id: &str,
    request: &mut tiny_http::Request,
...
fn http_handle_debug_disconnect(
    store: &StateStore,
    serve_state: &Arc<Mutex<DapRegistry>>,
    session_id: &str,
    request: &mut tiny_http::Request,
...
fn http_handle_debug_terminate(
    store: &StateStore,
    serve_state: &Arc<Mutex<DapRegistry>>,
    session_id: &str,
    request: &mut tiny_http::Request,
...
fn http_handle_debug_adopt_subprocess(
    store: &StateStore,
    serve_state: &Arc<Mutex<DapRegistry>>,
    session_id: &str,
    request: &mut tiny_http::Request,
