...
#[test]
fn inspect_clamps_tail_lines_to_safe_upper_bound() {
    if !python_available() {
        return;
    }

    let tmp = tempdir().expect("temp dir should exist");
    let script_path = tmp.path().join("app_tail_bound.py");
    fs::write(
        &script_path,
    ...
    let payload =
        String::from_utf8(inspect_assert.get_output().stdout.clone()).expect("inspect output utf8");
    let doc: Value = serde_json::from_str(&payload).expect("inspect output should be json");
    assert_eq!(doc["log"]["tail_lines"].as_u64(), Some(5000));
