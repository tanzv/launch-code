...
#[test]
fn serve_supports_batched_dap_requests() {
    ...
    let dap_thread = thread::spawn(move || {
        ...
        for (seq_out, expected_cmd) in [(1u64, "initialize"), (2u64, "threads")] {
            let msg = read_dap_message(&mut reader);
            assert_eq!(msg["type"], "request");
            assert_eq!(msg["command"], expected_cmd);
    ...
    assert_eq!(json["responses"][0]["type"], "response");
    assert_eq!(json["responses"][0]["command"], "initialize");
    assert_eq!(json["responses"][1]["type"], "response");
    assert_eq!(json["responses"][1]["command"], "threads");
