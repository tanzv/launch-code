...
#[test]
fn serve_rejects_zero_max_events_and_allows_followup_event_poll() {
    let dap_listener = TcpListener::bind("127.0.0.1:0").expect("dap listener should bind");
    let dap_port = dap_listener.local_addr().unwrap().port();
    ...
    let mut bad_events_res = agent
        .get(&format!(
            "{url}/v1/sessions/session-1/debug/dap/events?timeout_ms=1000&max=0"
        ))
        .header("Authorization", "Bearer testtoken")
        .call()
        .expect("invalid events request should complete");
    ...
    assert!(
        events.iter().any(|item| item["event"] == "stopped"),
        "stopped event should be present after invalid query"
    );
