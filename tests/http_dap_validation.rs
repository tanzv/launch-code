...
#[test]
fn serve_rejects_empty_dap_commands_and_allows_followup_request() {
    let dap_listener = TcpListener::bind("127.0.0.1:0").expect("dap listener should bind");
    let dap_port = dap_listener.local_addr().expect("local addr").port();
    ...
    let mut bad_command_res = agent
        .post(&format!("{url}/v1/sessions/session-1/debug/dap/request"))
        .header("Authorization", "Bearer testtoken")
        .send(serde_json::to_string(&json!({"command":"   ","arguments":{}})).unwrap())
        .expect("bad dap request should complete");
    assert_eq!(
        bad_command_res.status(),
        ureq::http::StatusCode::BAD_REQUEST
    );
    let bad_command_doc: Value = serde_json::from_str(
        &bad_command_res
            .body_mut()
            .read_to_string()
            .expect("bad command response should be readable"),
    )
    .expect("bad command response should be json");
    assert_eq!(bad_command_doc["ok"], false);
    assert_eq!(bad_command_doc["error"], "bad_request");
    assert_eq!(bad_command_doc["message"], "command is required");

    let mut bad_batch_res = agent
        .post(&format!("{url}/v1/sessions/session-1/debug/dap/request"))
        .header("Authorization", "Bearer testtoken")
        .send(
            serde_json::to_string(&json!({
        ...
        .expect("bad batch request should complete");
    assert_eq!(bad_batch_res.status(), ureq::http::StatusCode::BAD_REQUEST);
    let bad_batch_doc: Value = serde_json::from_str(
        &bad_batch_res
            .body_mut()
            .read_to_string()
            .expect("bad batch response should be readable"),
    )
    .expect("bad batch response should be json");
    assert_eq!(bad_batch_doc["ok"], false);
    assert_eq!(bad_batch_doc["error"], "bad_request");
    assert_eq!(bad_batch_doc["message"], "batch items require command");
    ...
    let mut good_res = agent
        ...
        .expect("good dap request should complete");
    assert_eq!(good_res.status(), ureq::http::StatusCode::OK);
    let good_doc: Value = serde_json::from_str(
        &good_res
            .body_mut()
            .read_to_string()
            .expect("good response should be readable"),
    )
    .expect("good response should be json");
    assert_eq!(good_doc["ok"], true);
    assert_eq!(good_doc["response"]["command"], "threads");
...
#[test]
fn serve_rejects_combined_dap_batch_and_command_and_allows_followup_request() {
    let dap_listener = TcpListener::bind("127.0.0.1:0").expect("dap listener should bind");
    let dap_port = dap_listener.local_addr().expect("local addr").port();
    ...
    let mut bad_res = agent
        ...
        .expect("bad dap request should complete");
    assert_eq!(bad_res.status(), ureq::http::StatusCode::BAD_REQUEST);
    let bad_doc: Value = serde_json::from_str(
        &bad_res
            .body_mut()
            .read_to_string()
            .expect("bad response should be readable"),
    )
    .expect("bad response should be json");
    assert_eq!(bad_doc["ok"], false);
    assert_eq!(bad_doc["error"], "bad_request");
    assert_eq!(bad_doc["message"], "command cannot be combined with batch");
